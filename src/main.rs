...
use actix_cors::Cors;
use actix_web::{ HttpServer,  App, HttpResponse, HttpRequest,web,http::header::CONTENT_LENGTH };
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(|| {
        ...
        App::new()
            ...
            .route("/transcript", web::post().to(transcript))
            .route("/print_text", web::post().to(print_text))
    })
    .bind(("0.0.0.0", 8080))?
    .run()
    .await
...
async fn upload(mut payload: Multipart, req: HttpRequest) -> HttpResponse {
 ...
 async fn transcript(mut payload: Multipart, req: HttpRequest) -> HttpResponse {
    ...
    if let Ok(Some(mut field)) = payload.try_next().await {
        ...
        if filetype.is_some() && legal_filetypes.contains(&filetype.unwrap()) && field.name() == "avatar" {
            ...
            let full_dialog = all_transcripts.join("\n");
            println!("Retornando texto completo"); 
            return HttpResponse::Ok().body(full_dialog)
 ...
 async fn print_text(req: HttpRequest, info: web::Json<Info>) -> HttpResponse {
    ...
    if let Some(_auth_header) = req.headers().get("Authorization") {
        ...
        // Start the chat using the received text
        println!("Starting the chat...");
        let client = openai_rust::Client::new(&std::env::var("OPENAI_API_KEY").unwrap());
        let prompt = format!("summarize the following dialogue:\n{}", info.text);
        let args = ChatArguments::new("gpt-4", vec![
